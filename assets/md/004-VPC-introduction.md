...
| 0.0.0.0 / 0       | NAT-GW-1b | This is the less specific route. When any component (service) **_from inside the private subnet 1c_** (where table route is assigned) is demanding to connect to the ip-range (**destination = 0.0.0.0 / 0 --> Not 10.10.0.0 / 16**) then is sent through **_target = NAT-GW-1c_** (AWS NAT gateway). |
